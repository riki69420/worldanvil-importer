...
ARTICLES = str(ROOT / "out" / "articles.json")
...
with sync_playwright() as p:
    ...
    try:
        ...
        pop.wait_for_function("document.getElementById('count').textContent === '126'", timeout=5000)
        check(pop.locator("#msg").inner_text() == "Loaded 126 articles.", "popup loads 126 articles")
        ...
        check("0 / 126" in host.locator(".count").inner_text(), "panel shows 0 / 126")
        ...
        check("1 / 126" in host.locator(".count").inner_text(), "done count is 1 / 126 after fill")
        ...
        check("1 / 126" in h2.locator(".count").inner_text(), "done state persists across pages")
        second = articles[1]
        ...
        h2.locator(".search").fill("City of Myr")
        h2.locator(".row").first.locator(".t").click()
        d = h2.locator(".details").inner_text()
        check("Settlement" in d and "population" in d, "details show template + fields for City of Myr")
    finally:
